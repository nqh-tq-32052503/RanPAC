...
from dildatasets import get_dataset_names, get_dataset_config_names
...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    exp_group.add_argument('--permute_classes', type=binary_to_boolean_type, default=0,
                           help='Permute classes before splitting into tasks? This applies the seed before permuting if the `seed` argument is present.')
    exp_group.add_argument('--ncls_per_task', type=int, default=0, help="Number of classes for each task during training. If the dataset at current task has more than N classes, randomly choose N classes from them")
    exp_group.add_argument('--num_workers', type=int, default=4, help='Number of workers for the dataloaders (default=infer from number of cpus).')
