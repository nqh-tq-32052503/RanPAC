...
- A single dataset is defined in a file named `<dataset_name>.py` in the `dildatasets` folder.
...
from dildatasets.utils.continual_dataset import ContinualDataset

REGISTERED_DATASETS = dict()  # dictionary containing the registered dildatasets. Template: {name: {'class': class, 'parsable_args': parsable_args}}
...
def get_all_datasets_legacy():
    """
    Returns the list of all the available dildatasets in the dildatasets folder that follow the old naming convention.
    """

    return [model.split('.')[0] for model in os.listdir('dildatasets')
...
def get_dataset_names(names_only=False):
    ...
    If an error was detected while loading the available dildatasets, it raises the appropriate error message.

    Args:
        names_only (bool): whether to return only the names of the available dildatasets
    ...
    Returns:
        the named of the available continual dildatasets
    """
    ...
    def _dataset_names():
        ...
        for dataset in get_all_datasets_legacy():  # for the dildatasets that follow the old naming convention, load the dataset class and check for errors
            if dataset in names:  # dataset registered with the new convention has priority
                continue

            try:
                mod = importlib.import_module('dildatasets.' + dataset)
...
def get_dataset_config_names(dataset: str):
    ...
    The configurations are stored in the `dildatasets/configs/<dataset>` folder.
    """

    def _dataset_config_names(dataset):
        names = []
        if os.path.exists(f'dildatasets/configs/{dataset}'):
            names = [dset_config.split('.yaml')[0] for dset_config in os.listdir(f'dildatasets/configs/{dataset}')
...
def get_dataset_class(dataset_name, return_args=False) -> ContinualDataset:
    """
    ...
    if isinstance(names[dataset_name], Exception):
        raise names[dataset_name]
    if return_args:
        return names[dataset_name]['class'], names[dataset_name]['parsable_args']
    return names[dataset_name]['class']


def get_dataset(args, dataset_name: str) -> ContinualDataset:
    """
    ...
    """
    dataset_class, dataset_args = get_dataset_class(dataset_name, return_args=True)
    try:
...
# import all files in the `dildatasets` folder to register the dildatasets
for file in os.listdir(os.path.dirname(__file__)):
    if file.endswith('.py') and file != '__init__.py':
        importlib.import_module(f'dildatasets.{file[:-3]}')
